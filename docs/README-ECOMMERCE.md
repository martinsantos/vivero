...
- **27 Oct 2025 – Layout de tienda con sidebar**
  - **Problema**: el CSS de `assets/css/tienda-sidebar.css` aplicaba `display:flex` sobre `.woocommerce`, provocando que el header se comprimiera contra la izquierda en `/tienda/`.
  - **Solución**: se encapsuló el loop principal en `<div class="shop-main">`, se llamó a `do_action('woocommerce_sidebar')` dentro del wrapper y se limitó el flexbox a `.shop-archive-wrap` para que el header conserve su ancho completo.
  - **Testing**: recorrido completo en `/tienda/` (desktop y mobile). Verificado header estable, grilla de productos sin desplazamientos y sidebar fijo sin superponer contenido cuando se habilita.

- **27 Oct 2025 – Mensajes de WooCommerce y títulos de producto**
  - **Problema**: emojis rotos en mensajes de "agregado al carrito", títulos de producto con formato técnico (guiones, mayúsculas inconsistentes), falta de espaciado en páginas de producto.
  - **Solución**: 
    - Agregado filtro `wc_add_to_cart_message_html` para remover emojis problemáticos y reemplazarlos con iconos Material.
    - Agregado filtro `the_title` para formatear títulos de producto (capitalización correcta, reemplazo de guiones/underscores).
    - Mejorado CSS de mensajes WooCommerce con colores, espaciado y botones estilizados.
  - **Testing**: verificado en páginas de producto que los mensajes se muestran correctamente con iconos, títulos legibles y espaciado apropiado.

- **27 Oct 2025 – Implementación de bocetos originales**
  - **Análisis**: Revisión completa de todos los bocetos vs implementación actual (ver `ANALISIS_BOCETOS_VS_PRODUCCION.md`)
  - **Cambios implementados**:
    - **Sidebar de filtros mejorado** (`tienda-sidebar-enhanced.css`): Checkboxes personalizados con estilo boceto, price slider visual mejorado, colores y tipografía según diseño original (#1dc91d, Epilogue font)
    - **Ordenamiento visual** (`shop-ordering.css`): Botones de ordenamiento estilizados, barra de búsqueda prominente en tienda, result count mejorado
    - **Paleta de colores actualizada**: Primary #1dc91d, backgrounds #f6f8f6, borders #e4e8e4, text #112111
  - **Testing**: Verificado en `/tienda/` que los filtros, ordenamiento y búsqueda se muestran según bocetos originales
