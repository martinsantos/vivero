...
<head>
    ...
    if (is_singular()) {
        ...
        $canonical_url = home_url(add_query_arg(array()));
    }
    echo '<link rel="canonical" href="' . esc_url($canonical_url) . '" />';

    // Get current season for meta description
    $month = date('n');
    $season = '';
    $season_desc = '';
    ...
    if ($month >= 3 && $month <= 5) {
        $season = 'Otoño';
        $season_desc = 'Descubre nuestra colección de otoño con plantas resistentes a temperaturas medias y consejos de jardinería estacional.';
    } elseif ($month >= 6 && $month <= 8) {
        $season = 'Invierno';
        $season_desc = 'Explora nuestra selección de invierno con plantas de interior y consejos para proteger tu jardín del frío.';
    } elseif ($month >= 9 && $month <= 11) {
        $season = 'Primavera';
        $season_desc = 'Celebra la primavera con nuestra nueva colección de flores y plantas decorativas. ¡Es momento de renovar tu jardín!';
    } else {
        $season = 'Verano';
        $season_desc = 'Encuentra plantas resistentes al calor y consejos de riego para mantener tu jardín fresco este verano.';
    }
    ?>
    <meta name="description" content="<?php echo esc_attr($season_desc); ?>">
    <?php wp_head(); ?>
    <link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/css/tailwind.min.css">
