        ...
        <div class="products-grid-featured">
            ...
            if (class_exists('WooCommerce')) {
                ...
                if ($featured_products) {
                    ...
                    // Función para obtener la imagen destacada del producto (con fallback SVG)
                    function get_home_category_image($product_id) {
                        $product = wc_get_product($product_id);
                        if (!$product) {
                            if ( function_exists('loscocos_get_product_image_url') ) {
                                $maybe = loscocos_get_product_image_url($product_id, 'woocommerce_thumbnail');
                                if ($maybe) { return $maybe; }
                            }
                            return wc_placeholder_img_src('woocommerce_thumbnail');
                        }
                        
                        // Preferir helper global con SVG fallback si existe
                        if ( function_exists('loscocos_get_product_image_url') ) {
                            $maybe = loscocos_get_product_image_url($product_id, 'woocommerce_thumbnail');
                            if ($maybe) { return $maybe; }
                        }
                ...
                if ($seasonal_products) {
                    ...
                    foreach ($seasonal_products as $product_post) {
                        ...
                        if ($product && $count < 6) {
                            ...
                            echo '<div class="product-card-standard" data-product-id="' . $product->get_id() . '" data-product-url="' . $product_url . '">';
                            
                            // Imagen del producto
                            echo '<div class="product-image" onclick="navigateToProduct(' . $product->get_id() . ')">';
                            ...
                            if ($product->is_in_stock()) {
                                echo '<button class="btn-add-to-cart-featured" data-product-id="' . $product->get_id() . '" data-product-name="' . esc_attr($product->get_name()) . '" onclick="event.stopPropagation(); addToCartFeatured(' . $product->get_id() . ', \'' . esc_js($product->get_name()) . '\');">';
                            ...
                            echo '</div>'; // product-card-standard
                        }
                    }
                }
            }
...
// Lógica de carrito delegada a js/cart-woocommerce.js

// Función para agregar productos al carrito desde cards destacadas
function addToCartFeatured(productId, productName) {
    console.log('🎆 Agregando producto destacado:', productName, 'ID:', productId);
    
    // Buscar el botón correspondiente
    const button = document.querySelector(`[data-product-id="${productId}"].btn-add-to-cart-featured`);
    if (!button) {
        console.error('❌ Botón no encontrado para producto:', productId);
        return;
    }
    ...
    // Simular agregado al carrito (reemplazar con tu lógica real de WooCommerce)
    setTimeout(() => {
...
function navigateToProduct(productId) {
    const card = document.querySelector(`[data-product-id="${productId}"].product-card-standard`);
...
function showCartNotificationFeatured(productName, success = true) {
    // Remover notificaciones existentes
    const existingNotifications = document.querySelectorAll('.cart-notification-featured');
    existingNotifications.forEach(notification => notification.remove());
    
    // Crear nueva notificación
    const notification = document.createElement('div');
    notification.className = 'cart-notification-featured';
        ...
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
                <div style="font-size: 14px; margin-bottom: 4px;">
                    ${success ? '¡Producto agregado al carrito!' : 'Error al agregar producto'}
                </div>
...
function updateCartCount() {
    // Aquí puedes agregar la lógica real para actualizar el contador
    // Por ejemplo, hacer una petición AJAX para obtener el número actual de productos
