			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="product-remove">
							<?php
								echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									...
									sprintf(
										'<a role="button" href="%s" class="remove" aria-label="%s" data-product_id="%s" data-product_sku="%s"><span class="material-icons" aria-hidden="true">close</span></a>',
						...
						echo '<div class="qty-control" data-cart-key="' . esc_attr( $cart_item_key ) . '">';
						echo '<button type="button" class="qty-btn minus" data-action="minus" data-cart-key="' . esc_attr( $cart_item_key ) . '" aria-label="' . esc_attr__( 'Decrease quantity', 'loscocos' ) . '"><span class="material-icons" aria-hidden="true">remove</span></button>';
						echo apply_filters( 'woocommerce_cart_item_quantity', $product_quantity, $cart_item_key, $cart_item ); // PHPCS: XSS ok.
						echo '<button type="button" class="qty-btn plus" data-action="plus" data-cart-key="' . esc_attr( $cart_item_key ) . '" aria-label="' . esc_attr__( 'Increase quantity', 'loscocos' ) . '"><span class="material-icons" aria-hidden="true">add</span></button>';
						echo '</div>';
						?>
						</td>
